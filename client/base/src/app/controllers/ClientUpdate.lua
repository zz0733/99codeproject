				...
				if jsondata then
					...
					if nil == newlist and nil ~= this._listener.updateResult then
						if true == isDebug() then
							this._listener:updateResult(false, this._newfileurl .." 获取服务器列表失败！md5文件列表为空!")
						else
					...
					for k,v in pairs(oldlist) do
						...
						if  oldpath then
							...
							--删除文件
							if bdel == true then
								local delpath = this._savepath..oldpath..oldname
					...
					for k ,v in pairs(newlist) do
						...
						if newpath then
							...
							if needupdate == true then
								...
								-- 先移除
								local delpath = this._savepath..newpath..newname
					...
					if #this._downList >0 then
						...
						cc.FileUtils:getInstance():writeStringToFile(this._response,this._curfile)
						this._listener:updateResult(true,"")
					end
				else
					if true == isDebug() then
						this._listener:updateResult(false, this._newfileurl .." 更新列表为空!")
					else
						this._listener:updateResult(false,"获取服务器列表失败！")
					end
				end
...
function ClientUpdate:UpdateFile()
	...
	downFileAsync(url,fileinfo[2],dstpath,function(main,sub)
			...
			if main == appdf.DOWN_PRO_INFO then --进度信息
				this._listener:updateProgress(sub,fileinfo[2], (this._downIndex / (#this._downList + 1)) * 100)
			elseif main == appdf.DOWN_COMPELETED then --下载完毕
				...
				if sub == 28 and this._retryCount > 0 then
					...
					if true == isDebug() then
						this._listener:updateResult(false,"文件 [" .. fileinfo[2] .. "] 下载失败,main:" .. main .. " ## sub:" .. sub) --失败信息
					else
