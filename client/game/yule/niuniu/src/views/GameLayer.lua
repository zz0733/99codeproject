...
function GameLayer:onEventGameMessage(sub,dataBuffer)   --接收消息通知
    ...
    elseif sub == "ask_banker" then      --该谁抢庄
        self._gameView:onEventAsk_banker(dataBuffer)
    elseif sub == "start_chip" then      --开始叫分
        self._gameView:onEventStart_chip(dataBuffer)
    elseif sub == "deal" then      --发牌
        self._gameView:onEventDeal(dataBuffer)
    elseif sub == "commit" then     --对方摊牌
        self._gameView:onEventCommit(dataBuffer)
    elseif sub == "game_over" then     --结算
        self._gameView:onEventGameover(dataBuffer)
    elseif sub == "deskover" then     --结束
