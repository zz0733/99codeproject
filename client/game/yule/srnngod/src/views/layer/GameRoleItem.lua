...
local popPosition = {}
popPosition[cmd.LEFT_VIEWID] = cc.p(220, 214)
popPosition[cmd.RIGHT_VIEWID] = cc.p(642, 214)
popPosition[cmd.MY_VIEWID] = cc.p(115, 60)
local popAnchor = {} 
...
function GameRoleItem:switeGameState( isBanker )
    --if self.m_nViewId ~= cmd.MY_VIEWID then
        ...
        if nil == self.m_clipNick then            
            ...
            self.m_clipNick:setPosition(0, 60)
        ...
        if nil == self.m_clipScore then
            self.m_clipScore = ClipText:createClipText(cc.size(100, 20), lScore .. "")
            self.m_clipScore:setAnchorPoint(cc.p(0.5, 0.5))
            self.m_clipScore:setPosition(70, 15.5)
