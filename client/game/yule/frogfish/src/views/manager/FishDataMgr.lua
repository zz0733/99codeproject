...
local cmd = appdf.req("game.yule.frogfish.src" .. ".models.CMD_Game")
...
function FishDataMgr:getFishSpeed(frogFishKind)
...
function FishDataMgr:getFishBoxheight(frogFishKind)

--    local ret = 12
--    if frogFishKind == nil then 
--          return ret
--    end 
    if frogFishKind <= FishKind.FISH_KIND_COUNT then
...
function FishDataMgr:getFishRadius(frogFishKind)
...
function FishDataMgr:getFishMultiple(frogFishKind)
...
FishDataMgr.FISH_OF_Armature = {                    -- 注释是对前边的键值注释
    ...
    [FishKind.FISH_HAITUN]        = "yu20_buyu",    -- 25 海豚 
    [FishKind.FISH_ZHANGYU]       = "yu21_buyu",    -- 32 章鱼
    [FishKind.FISH_XIAOYINLONG]   = "yu22_buyu",    -- 22 小银龙
    [FishKind.FISH_XIAOJINLONG]   = "yu23_buyu",    -- 23 小金龙
    [FishKind.FISH_MEIRENYU]      = "yu24_buyu",    -- 20 美人鱼
    [FishKind.FISH_SHENXIANCHUAN] = "yu25_buyu",    -- 19 神仙船
    [FishKind.FISH_JINCHAN]       = "goldfrog",     -- 18 金蝉
    [FishKind.FISH_FOSHOU]        = "yu26_buyu",    -- 26 佛手
    [FishKind.FISH_BGLU]          = "yu27_buyu",    -- 27 炼丹炉
    ...
    [FishKind.FISH_PIECE]         = "jinyumantang_nuyu",            -- 31 金玉满堂 
}
...
function FishDataMgr:setUserScore(index, score)
  self._llUserScore[index+1] = score
end
