...
function FishPaoView:initCCB()
    ...
    --炮台界面
    self.m_pathUI = cc.CSLoader:createNode("game/frogfish/csb/gui-fish-pao.csb")
        ...
        if BulletCount < BULLET_NUM_MAX then
            if v.TouchPhase or v.AutomaticFiring then
                if v.is_shoot_stop == false then
                    v.firingInterval  =  v.firingInterval or cmd.firingIntervalEnum.normal
...
function FishPaoView:showWanFoChaoZhongEffect(_pos)
    ...
    local armature = ccs.Armature:create("yu26_buyu")
...
function FishPaoView:showLianDanLuEffect(_pos, tag)
    ...
    if tag == 0 then
        -- 镇妖塔
        ExternalFun.playGameEffect(FishRes.SOUND_OF_ZHENGYAOJINGANGTA)
        self:showZhenYaoTaEffect(_pos)
        print("---------------------------------------------镇妖塔")

    elseif tag == 1 then
        -- 无敌风火轮
        ExternalFun.playGameEffect(FishRes.SOUND_OF_WUDIFENGHUOLUN)
        self:showFengHuoLunEffect(_pos)
        print("---------------------------------------------无敌风火轮")
    elseif tag == 2 then
        -- 如意金箍棒
        ExternalFun.playGameEffect(FishRes.SOUND_OF_RUYIJINGUBANG)
        self:showJinGuBangEffect(_pos)
        print("---------------------------------------------如意金箍棒")
    end
end                 
...
function FishPaoView:showZhenYaoTaEffect(_pos)
    ...
    --镇妖塔
    local armature1 =  ccs.Armature:create("zhengyaotadonghua_buyu")
    armature1:getAnimation():play("move")
    armature1:setPosition(_pos)
    local function animation1Event(armature,moveMentType,moveMentId)
        if moveMentType == ccs.MovementEventType.complete
        or moveMentType == ccs.MovementEventType.loopComplete
        then
            if moveMentId == "move" then 
                self:removeArmature(armature)
            end 
        end 
    end
    armature1:getAnimation():setMovementEventCallFunc(animation1Event)
    armature1:addTo(self.m_pathUI, 90)

    --定
    local armature2 =  ccs.Armature:create("ding_buyu")
    armature2:getAnimation():play("die")
...
function FishPaoView:showFengHuoLunEffect(_pos)
    ...
    local armature = ccs.Armature:create("wudifenghuolun_buyu")
    ...
    local delay1 = cc.DelayTime:create(1.25)
    local callFunc1 = cc.CallFunc:create(function()
        local partical = cc.ParticleSystemQuad:create(FishRes.PARTICLE_OF_WDFHLBAOZHA)
        partical:setPosition(_pos)
        partical:setTag(200)
        self.m_pathUI:addChild(partical, 90 + 2)
    end)
    local delay2 = cc.DelayTime:create(1.75)
    local callFunc2 = cc.CallFunc:create(function()
        self.m_pathUI:removeChildByTag(200)
    end)
    self:runAction(cc.Sequence:create(delay1, callFunc1, delay2, callFunc2))
end

-- 如意金箍棒
function FishPaoView:showJinGuBangEffect(_pos)
    local armature = ccs.Armature:create("ruyijingubangdonghua_buyu")
    armature:getAnimation():play("move")
    armature:setPosition(_pos)
    local function animationEvent(armature,moveMentType,moveMentId)
        if moveMentType == ccs.MovementEventType.complete  then 
    ...
    for i = 1, BOMB_ITEM_COUNT / 2 do
        ...
        self.m_pathUI:addChild(bombItem, 90)
    end
    ...
    self:runAction( cc.Sequence:create(cc.DelayTime:create(0.15), callFunc))
end
