...
function FishPaoView:initCCB()
    ...
    --炮台界面
    self.m_pathUI = cc.CSLoader:createNode("game/monkeyfish/csb/gui-fish-pao.csb")
...
function FishPaoView:showWanFoChaoZhongEffect(_pos)
    ...
    BombManager:createBombWithType(self, _pos, BOMB_TYPE_FOZHANG)
    print("----------------------------万佛朝中 ")
end
...
function FishPaoView:showZhenYaoTaEffect(_pos)
    ...
    -- 定
    local strArmName = {"zhengyaota_buyu", "dingpao_buyu", "zhengyaotawenzidonghua_buyu", }

    for i = 1, 3 do
        local armature2 = ccs.Armature:create(strArmName[i])
        armature2:getAnimation():play("Animation1")
        armature2:setPosition(_pos)
        local function animationEvent(armature,moveMentType,moveMentId)
            if moveMentType == ccs.MovementEventType.complete 
            or moveMentType == ccs.MovementEventType.loopComplete
            then 
        ...
        armature2:getAnimation():setMovementEventCallFunc(animationEvent)
...
function FishPaoView:showFengHuoLunEffect(_pos)
    ...
    local strArmName = {"wudifenghuolun_buyu", "wudifenghuolunwenzidonghua_buyu", }
    ...
    for i = 1, 2 do
        ...
        armature:getAnimation():setMovementEventCallFunc(animationEvent)
        armature:addTo(self.m_pathUI, 90 + 1)
    end

    --显示粒子
    local delay1 = cc.DelayTime:create(1.25)
    local callFunc1 = cc.CallFunc:create(function()
        local partical = cc.ParticleSystemQuad:create(FishRes.PARTICLE_OF_FENGHUOLUN)
        partical:setPosition(_pos)
        partical:setTag(200)
        self.m_pathUI:addChild(partical, 90 + 2)
    end)
...
function FishPaoView:showJinGuBangEffect(_pos)
    ...
    BombManager:createBombWithType(self, _pos, BOMB_TYPE_JINGB)
    print("----------------------------金箍棒 ")
end
