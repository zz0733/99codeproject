...
function GameRoomLayer:fillRoomCellRes()
   self.m_roomCellRes ={
        --体验场
        {[cell_bg]    = "tyc.png",
        [cell_title] = "tyc_xctitle.png",
        [cell_icon]  = "tyc_icon.png",
        [cell_dTextbg]  = "text_mfsw.png"},
        --海滨渔场
        {[cell_bg]    = "roomBtnbg.png",
        [cell_csbani] = "hbyc.csb",
        [cell_aniname] = "hbyc",},
        --黄金海岸
        {[cell_bg]    = "roomBtnbg.png",
        [cell_csbani] = "hjha.csb",
        [cell_aniname] = "hjha"},
        --海底宝藏
        {[cell_bg]    = "roomBtnbg.png",
        [cell_csbani] = "hdbz.csb",
        [cell_aniname] = "hjha",},
    }
    self.gameTitle = "title.png"
end
...
function GameRoomLayer:createScrollRoomList()
   ...
   for index = 1, nRoomCount do
        ...
        --后三个播放动画
        if index  > 1 then
            local anipath = self.res_path..self.m_roomCellRes[index][cell_csbani]
            local aniname = self.m_roomCellRes[index][cell_aniname]
            ExternalFun.playUnLoadCSBani(anipath, aniname,_btnNode, true,ccp(0,0))
        end
        ...
        if index == 1 then
            ...
            title:setPosition(cc.p(bgSize.width/2, bgSize.height/2-70))
            ...
            diBg:setPosition(cc.p(bgSize.width/2, bgSize.height/2-168))
        ...
        lbLimitScore:setPosition(cc.p(142, 110));
        ...
        if index == 1 then
           lbEnterScore:setVisible(false)
           lbLimitScore:setVisible(false)
        end
...
function GameRoomLayer:onButtonClicked(pSender)
    ...
    local nIndex = pSender:getTag()
        pSender:getParent():runAction((cc.Sequence:create(
