...
GameLayer.Res = "game/yule/brnngod/res/"
...
-- 发送下注
function GameLayer:sendBet_in(tab_bet)
    local msgdata = {}
    msgdata["type"] = "nn100"
    msgdata["tag"] = "bet"
    msgdata["body"] = tab_bet
    local strvalue = cjson.encode(msgdata)
    self._gameFrame:sendSocketData(strvalue)
end
-- 发送上庄
function GameLayer:sendUpDealer()
    local msgdata = {}
    msgdata["type"] = "nn100"
    msgdata["tag"] = "updealer"
    local strvalue = cjson.encode(msgdata)
    self._gameFrame:sendSocketData(strvalue)
end
-- 发送取消上庄
