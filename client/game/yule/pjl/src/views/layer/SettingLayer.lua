...
function SettingLayer:init()
    ...
    --加载csb资源
    self.m_pathUI = cc.CSLoader:createNode("shuihuzhuan/SHZ_Setup.csb")
    self.m_pathUI:addTo(self.m_rootUI)

    self.center = self.m_pathUI:getChildByName("node_center")
    self.center:setPositionX(display.width / 2)
    self.closebtn = self.center:getChildByName("button_btnClose")
