...
GameLayer.Res = "game/yule/pjl/res/"
local WaterMarginDataMgr        = appdf.req(module_pre..".views.manager.WaterMarginDataMgr")
local WaterMarginScene_Events   = appdf.req(module_pre..".views.scene.WaterMarginSceneEvent")
...
--离开房间
function GameLayer:onExitRoom(Tag)--被踢出去/自己出去
    if Tag == 1 then
...
function GameLayer:sendStart()
    ...
    local nChangeBetMoney =  WaterMarginDataMgr.getInstance():getBaseBetChangeMul()
    local bet_ = nBaseBetGlod * nChangeBetMoney

    local msgdata = {}
    msgdata["type"] = "pjl"
    msgdata["tag"] = "start"
    msgdata["body"] = {9,bet_,self.session_id}
...
--    end
    ...
    elseif sub == "start" then        
        print("这次的中奖结果是___________________"..dataBuffer[1])
        dump(dataBuffer)
        self._gameView:onEventStart(dataBuffer)
        self:process_200_100(dataBuffer)
        
    elseif sub == "dice" then      --比倍结果
        print("这次的比倍结果是___________________")
        dump(dataBuffer)
        local diceMoney = dataBuffer[4]
        if dataBuffer[1] == false then
            diceMoney = - diceMoney
        end
        self._gameView:onEventDice(diceMoney)
...
--statt 消息 处理
function GameLayer:process_200_100(__data)
--    local _buffer = __data:readData(__data:getReadableSize())
    local _buffer = WaterMarginDataMgr.getInstance():getRandomResult(__data[1])
    --数据
    local msg = {}
    msg.llResultMoney = __data[2]
    msg.llResultScore = __data[5]
    msg.result_icons = {}   --3*5

    for i=1,tonumber(WATER_ICON_ROW),1 do           --3
        local array = {}   
        for j=1,tonumber(WATER_ICON_COL),1 do       --5
             array[j] = _buffer.tables[j][i]-1--_buffer:readInt()
        end
