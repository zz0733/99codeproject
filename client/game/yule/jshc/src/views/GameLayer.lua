...
-- 发送筹码
--========================================发送协议=============================================
--SendBet {哪个桌子, 哪个筹码} 投币筹码
function GameLayer:SendBet(tab_bet)
--    'type':'zero', 'tag':'bet', 'body':(下注区域0-7, *筹码额度)
    local msgta = {}
    msgta["type"] = "jshc"
    msgta["tag"] = "bet"
    msgta["body"] = tab_bet
...
--SendUpdealer {} 申请上庄
function GameLayer:SendUpdearler()
    local msgta = {}
    msgta["type"] = "jshc"
    msgta["tag"] = "updealer"
--    msgta["body"] = {}
...
--SendDowndealer{} 申请下庄
function GameLayer:SendDowndealer()
    local msgta = {}
    msgta["type"] = "jshc"
    msgta["tag"] = "downdealer"
    if type(msgta) == "table" then
       strvalue = cjson.encode(msgta)
    end
    self._gameFrame:sendSocketData(strvalue)
...
function GameLayer:onEventGameMessage(sub,dataBuffer)   --接收消息通知betmoneys{[1]=112 [2]=123 [3]=161 [4]=102 [5]=98 [6]=286 [7]=178 [8]=70 [9]=7 [10]=259 [11]=300 }
--     print(sub.."--------------------------------")
--    dump(dataBuffer,"***************************")
