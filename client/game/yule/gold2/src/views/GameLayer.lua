...
-- 发送跟注
function GameLayer:sendAdd(betNum)
...
--    dump(dataBuffer,"***************************")
    ...
    if sub == "enter" then          --进入游戏
        self._gameView:onEventEnter(dataBuffer)

    elseif sub == "come" then       --有玩家进入房间
        self._gameView:onEventCome(dataBuffer)

    elseif sub == "leave" then      --有玩家离开房间----------------
        self._gameView:onEventLeave(dataBuffer)

    elseif sub == "ready" then      --有玩家准备
        self._gameView:onEventReady(dataBuffer)    

    elseif sub == "update_intomoney" then      --更新自身钱包
        self._gameView:onEventUpdate_intomoney(dataBuffer)

    elseif sub == "drop" then       --玩家弃牌
        self._gameView:onEventDrop(dataBuffer)

    elseif sub == "add" then        --跟注/加注
        self._gameView:onEventAdd(dataBuffer)

    elseif sub == "run" then        --发牌
        self._gameView:onEventRun(dataBuffer)

    elseif sub == "run2" then       --中途进入房间
        self._gameView:onEventRun2(dataBuffer)

    elseif sub == "show" then       --看牌
        self._gameView:onEventShow(dataBuffer)

    elseif sub == "vs" then         --比牌
        self._gameView:onEventVS(dataBuffer)

    elseif sub == "over" then       --结算
        self._gameView:onEventOver(dataBuffer)

    elseif sub == "deskover" then       --游戏结束
        self._gameView:onEventDeskover(dataBuffer)
    end
