...
function GameRoomLayer:init(bNeedRoomList)
    ...
    local strTitlePath =  self.res_path.."zjh_xc_title.png"
...
function GameRoomLayer:createScrollRoomList()
    ...
    local nRoomCount = 3 --#self.m_vecRoomList
...
function GameRoomLayer:showAnimation()
	...
	local function clipNode(node, index)
		local pathStr = self.res_path.."%s.png"
		local nameStr = {"zjh_039","zjh_040","zjh_041"}
        local clipLigth = DTClippingNode:create(node, string.format(pathStr, nameStr[index]))
