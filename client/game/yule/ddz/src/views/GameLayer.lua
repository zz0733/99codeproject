...
--叫地主/抢地主
function GameLayer:SendCall(num)
    local msgta = {}
    msgta["type"] = "ddz"
    msgta["tag"] = "call"
    msgta["body"] = num
...
--加倍
function GameLayer:SendRaise(num)
    local msgta = {}
    msgta["type"] = "ddz"
    msgta["tag"] = "raise"
    msgta["body"] = num
...
-- 出牌
function GameLayer:SendPlay(paixing)
    local msgta = {}
    msgta["type"] = "ddz"
    msgta["tag"] = "play"
    msgta["body"] = paixing
...
-- 托管
function GameLayer:SendManage(isManage)
    local msgta = {}
    msgta["type"] = "game"
    msgta["tag"] = "manage"
    msgta["body"] = isManage
...
----        dump(dataBuffer)
    ...
    elseif sub == "leave" then                                 --该uid玩家离开房间  -- id
        print("该uid玩家离开房间")
        self._gameView:onUserLeave(dataBuffer)
    elseif sub == "call" then                                  --叫地主
        print("叫地主")
        self._gameView:onPlayerShow(dataBuffer)    
    elseif sub == "startraise" then                            --开始加倍
        print("开始加倍")
        self._gameView:StartRaise(dataBuffer)
    elseif sub == "raise" then                                 -- 加倍
        print("加倍")
        self._gameView:updataRaise(dataBuffer)
    elseif sub == "show" then                                  -- 明牌
        print("明牌")
        self._gameView:ShowCards(dataBuffer)
    elseif sub == "pass" then                                  -- 过牌
        print("过牌")
        self._gameView:PassCards(dataBuffer)
    elseif sub == "play" then                                  --出牌
        print("出牌")
        self._gameView:PlayCards(dataBuffer)
    elseif sub == "update" then                                --重进后刷新牌桌
        print("重进后刷新牌桌")
        self._gameView:updataDesk(dataBuffer)
    elseif sub == "manage" then                                -- 托管
        print("托管")
        self._gameView:updataManage(dataBuffer)
    elseif sub == "gameover" then      --结算
        print("结算时间")
        self._gameView:GameOverTime(dataBuffer)
    elseif sub == "update_intomoney" then      --更新自身玩家的背包金额
        print("跟新玩家背包金额")
        self._gameView:updataMyMoney(dataBuffer) 
    elseif sub == "deal" then
        print("发牌")
        self._gameView:updataFaPai(dataBuffer)
    elseif sub == "ready" then      --其他玩家准备id
        print(dataBuffer.."准备")
        self._gameView:updataReady(dataBuffer)
    elseif sub == "readycd" then      --玩家准备时间
        print("准备时间"..dataBuffer)
        self._gameView:updataReadyTime(dataBuffer)
    elseif sub == "notcontinue" then                 --???????  databuff = = nil  ???
        print("？？？？？？")
    elseif sub == "showcard_2" then
        print("")
        dump(dataBuffer,"******")
--        {[1]=true [2]=4 [3]={} }    {[1]=98771747 [2]=false [3]={} }
    elseif sub == "online" then
        dump(dataBuffer,"******")
    elseif sub == "allcards" then
        dump(dataBuffer,"******")
    elseif sub == "pong" then
        print("********************************")
    elseif sub == "deskover" then
        print(dataBuffer)
    elseif sub == "cards_fail" then
        print(dataBuffer)
    elseif sub == "taskfinish" then
        print(dataBuffer)
    elseif sub == "login_from_somewhere" then
        print(dataBuffer)
--    {room_title="斗地主免费场" room_id=1551 client_host="47.107.89.198" }
    else
        print(sub)
    end
end
