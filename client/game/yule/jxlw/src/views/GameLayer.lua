...
GameLayer.Res = "game/yule/jxlw/res/"
local TigerDataMgr          = appdf.req(module_pre..".views.manager.TigerDataMgr")
local TigerConst            = require(module_pre..".views.scene.Tiger_Const")
local vecCardLineDate       = TigerConst.vecCardLineDate -- 线数数据
local Tiger_Events          = require(module_pre..".views.manager.Tiger_Events")
local Tiger_Define          = require(module_pre..".views.scene.Tiger_Define")
...
function GameLayer:CreateView()
    return GameViewLayer:create(self)
        :addTo(self)
    -- local rootLayer, csbNode = ExternalFun.loadRootCSB("tongbinn/tongbinn_gameScene.csb", self)
    -- self.root = csbNode
    -- --隐藏所有的玩家panel
    -- for i = 1,6 do
    --     self.root:getChildByName("player_Panel_"..tostring(i)):setVisible(false)
    -- end
    -- return rootLayer
...
--离开房间
function GameLayer:onExitRoom( Tag )
    if Tag == 1 then
...
-- 发送开始
function GameLayer:sendStart(line,jetton)
    local msgdata = {}
    msgdata["type"] = "pjl"
    msgdata["tag"] = "start"
    msgdata["body"] = {line,jetton ,self.session_id}
...
-- 发送比倍
function GameLayer:sendDice(size_)--大小，1大 2小 3中 
    --self:KillGameClock()    --倒计时进度条 暂时还没写
    local msgdata = {}
    msgdata["type"] = "jxlw"
    msgdata["tag"] = "dice"
    msgdata["body"] = {size_,self.session_id_dice}
    local strvalue = cjson.encode(msgdata)
    self._gameFrame:sendSocketData(strvalue)

    self.session_id_dice = self.session_id_dice+1
end
-- 发送换桌
...
--    end
--    if sub ~= "rank" then
--        print(sub.."______________________________消息_____________")
--        dump(dataBuffer)
--    end
    ...
    if sub == "enter" then      --自身玩家进入房间    
        self._gameView:onEventEnter(dataBuffer)

    elseif sub == "pool" then   --有玩家进入房间
        self._gameView:onEventPool(dataBuffer)

    elseif sub == "start" then        
        print("这次的中奖结果是___________________")
        dump(dataBuffer)
        self._gameView:onEventStart(dataBuffer)
        self:process_game_start(dataBuffer)
        
    elseif sub == "dice" then      --比倍结果
--        print("这次的比倍结果是___________________")
--        dump(dataBuffer)
--        self:process_200_101(dataBuffer)

    elseif sub == "update_intomoney" then      --更新自身玩家的背包金额
        self._gameView:onEventUpdate_intomoney(dataBuffer)

    elseif sub == "come" then   --有玩家进入房间
        self._gameView:onEventCome(dataBuffer)


    elseif sub == "leave" then     --该uid玩家离开房间
--        self._gameView:onEventLeave(dataBuffer)
...
--statt 消息 处理
function GameLayer:process_game_start(__data)
    if __data[1] == 99 or __data[1] == 999 then
        print("我心飞翔_______________________")
    end
    if type(__data[1]) == "string" then
        return
    end

    local _buffer = TigerDataMgr.getInstance():getRandomResult(__data[1]) --通过 中奖倍数 在表中查找
    if TigerDataMgr:getInstance():getFreeTimes() > 0 then
        return
    end
    --数据
    local msg = {}
    msg.llResultMoney = __data[2]   --背包余额(本轮结算后)
    msg.llResult = __data[5]        --中奖金额
    msg.cbCardIndex = {}   --3*5
--    dump(_buffer.symbol_tables)
    for i=1,tonumber(WATER_ICON_ROW),1 do           --3
        for j=1,tonumber(WATER_ICON_COL),1 do       --5
            local iconIndex = self:numChange1_10(_buffer.symbol_tables[j][i])
            msg.cbCardIndex[(i-1)*WATER_ICON_COL+j] = iconIndex
--            print("_________________".._buffer.symbol_tables[j][i])
        end
...
function GameLayer:numChange1_10(num_)
    local _num = num_
    if _num == 1 then
        _num = 10
    elseif _num == 2 then
...
----dice 消息 骰子结果(比大小)
--function GameLayer:process_200_101(__data)
...
--    local msg1 = {}
--    msg1.llResultScore = __data[3]      --背包余额
--    msg1.dice_points = __data[2]        --筛子点数

--    --保存
--    WaterMarginDataMgr.getInstance():setGameCompareResult(msg1);                --设置比倍结果
--    WaterMarginDataMgr.getInstance():addGameCompareResult(msg1.llResultScore)   --存入历史比倍记录
