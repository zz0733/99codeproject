...
function GameRoomLayer:fillRoomCellRes()
   self.m_roomCellRes ={
        ...
        [cell_icon]  = "fruitslots_1.png",
        [cell_bgtop]  = "hbyc_3.png",
        [cell_dTextbg]  = "hbyc_2.png"},
        --中级场
        {[cell_bg]    = "hjha.png",
        [cell_title] = "blackjack_xctitle_gjc.png",
        [cell_icon]  = "fruitslots_5.png",
        [cell_bgtop]  = "hjha_3.png",
        [cell_dTextbg]  = "hjha_2.png"},
        --高级场
        {[cell_bg]    = "hdbz.png",
        [cell_title] = "blackjack_xctitle_zjc.png",
        [cell_icon]  = "fruitslots_6.png",
        [cell_bgtop]  = "hdbz_3.png",
        [cell_dTextbg]  = "hdbz_2.png"},
    }
    self.gameTitle = "title_room3.png"
end
...
-- 添加金币精灵
function GameRoomLayer:addIconsp(pSender)
    local pos = {ccp(45,165),ccp(230,220),ccp(50,360)}
    for index = 2,4 do
        local framename = self.res_path.."fruitslots_"..index..".png"
        local icoinsp = cc.Sprite:create(framename)
        icoinsp:setPosition(pos[index-1])
        :addTo(pSender)
    end
end
...
function GameRoomLayer:setjackPotText()
    ...
    local mtable = {}
    local ptable = {math.random(85000,100000),math.random(400000,500000),math.random(600000,999999),}
    for i = 1,self.roomNum do
       mtable[i] = ptable[i]
       local finalnum = ExternalFun.formatnumberthousands(ptable[i])
       self.jackPotText[i]:setString(finalnum)
    end
    --下注数随机自增每两秒
    local function updatejackpot(args)
        local randtaget = math.random(1,3)
