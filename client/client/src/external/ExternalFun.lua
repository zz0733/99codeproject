...
function ExternalFun.numberTransiform(strCount)
   local numStr = tonumber(strCount)
   if math.floor(numStr) < numStr then 
       return  ExternalFun.numberTransiformfloat(strCount)
   else 
       return  ExternalFun.numberTransiformint(strCount) 
   end
end
--人民币阿拉伯数字转大写(整数)
function ExternalFun.numberTransiformint(strCount)
...
--人民币阿拉伯数字转大写（支持两位小数）
function ExternalFun.numberTransiformfloat(strCount)
    local numStr = tonumber(strCount)
    local cn_num = {"零","壹","贰","叁","肆","伍","陆","柒","捌","玖"}
    local jiaoNum = math.floor((numStr*10)%10)
    local fenNum = math.floor((numStr*100)%10)
    local strJiao = tostring(jiaoNum)
    local strFen = tostring(fenNum)

    local strYuanzi = math.floor(numStr) >0 and "元" or ""
    local strJiaozi = jiaoNum > 0 and "角" or ""
    local strFenzi = fenNum > 0 and "分" or ""

    local JiaoNumstr = cn_num[string.byte(strJiao, i) - 47]
    local FenNumstr = cn_num[string.byte(strFen, i) - 47]

    if FenNumstr == "零" then 
        FenNumstr = "" 
    end

    if JiaoNumstr == "零" and math.floor(numStr) == 0 then 
        JiaoNumstr = "" 
    end

    local strEx = strYuanzi..JiaoNumstr..strJiaozi..FenNumstr..strFenzi
    return  ExternalFun.numberTransiformint(math.floor(strCount))..strEx
end
...
function ExternalFun.playSoundEffect( path, useritem )
	...
	if nil == useritem then
		...
		-- 0:女/1:男
		local gender = 1--useritem.cbGender
...
--播放游戏音效
function ExternalFun.playGameEffect( path )
	local sound_path = path
	if GlobalUserItem.bSoundAble then
		AudioEngine.playEffect(sound_path,false)
	end	
...
--关闭音效
function ExternalFun.stopGameEffect( handle )
    if handle then
...
function ExternalFun.playClickEffect()
	if GlobalUserItem.bSoundAble then
		AudioEngine.playEffect(cc.FileUtils:getInstance():fullPathForFilename("sound/sound-button.mp3"),false)
	end
end
...
--播放背景音乐
function ExternalFun.playGameBackgroundAudio( bgfile )
	if GlobalUserItem.bVoiceAble then
		AudioEngine.playMusic(bgfile,true)
	end	
end
...
--播放大厅背景音乐
function ExternalFun.playLogonBackgroudAudio( )
	if GlobalUserItem.bVoiceAble then
		AudioEngine.playMusic(cc.FileUtils:getInstance():fullPathForFilename("sound/audio-login.mp3"),true)
	end
end
...
function ExternalFun.seekNodeByName(node,key)
    if node == nil then
        return nil;
    end
    -- print("------------key:"..key);
    ...
    for _, v in ipairs(childrenTab) do
        local res = ExternalFun.seekNodeByName(v,key);
        if res then
            --todo
            resultNode = res
            break
    ...
    if resultNode then
        --todo
        node.seekNodeMap[key] = resultNode
    end
    return resultNode;
end

-- 播放csb动画
function ExternalFun.playCSBani(path, actionName, root, isloop)
...
function ExternalFun.playUnLoadCSBani(path, actionName, root, isloop,pos)
    ...
    local HeroHand = cc.CSLoader:createNode(path)
    local HeroTimeline = cc.CSLoader:createTimeline(path)
    HeroTimeline:play(actionName, isloop)
    HeroHand:runAction( HeroTimeline )
    HeroHand:setPosition(pos)
    root:addChild( HeroHand )
end
