...
function ClipText:setHorizontalAlignment(setAlignment)
	if nil == self.m_text then
		return
	end
	self.m_text:setTextHorizontalAlignment(setAlignment)
end

function ClipText:setVerticalAlignment(setAlignment)
	if nil == self.m_text then
		return
	end
	self.m_text:setTextVerticalAlignment(setAlignment)
end
...
function ClipText:stringEllipsis(szText, config, maxWidth, replaceStr)
	...
	while true do
		...
		if byte > 128 then
			if width <= maxWidth - 3 * config.lowerEnSize.width then
