...
function Bridge_ios.testRecordService( )
    print(" Bridge_ios.testRecordService ")
end

function Bridge_ios.isNetworkAvailable()
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS, "isNetworkAvailable")
    if not ok then
        local msg = "luaoc error:" .. ret
        print(msg)  
        return false, msg  
...
function Bridge_ios.goToSetting()
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS, "goToSetting")
    if not ok then
        local msg = "luaoc error:" .. ret
        print(msg)  
        return false, msg  
...
function Bridge_ios.getLaunchData()
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS, "getLaunchData")
    if not ok then
        local msg = "luaoc error:" .. ret
        print(msg)  
        return ""  
...
function Bridge_ios.getCopyBoardData( callback )
    local args = { scriptHandler = callback }
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS, "getCopyBoardData", args)
    if not ok then
        local msg = "luaoc error:" .. ret
        print(msg)  
        return ""  
...
function Bridge_ios.getClientPackageName()
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS, "getClientPackageName")
    if not ok then
        local msg = "luaoc error:" .. ret
        print(msg)  
        return ""  
...
function Bridge_ios.getDeviceToken()
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS, "getDeviceToken")
    if not ok then
        local msg = "luaoc error:" .. ret
        print(msg)  
        return ""  
...
function Bridge_ios.systemCall( szTel )
    local paramtab = {msg = szTel}
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS,"systemCall", paramtab)
    if not ok then
        local msg = "luaoc error:" .. ret
        print(msg)  
        return 0, msg   
