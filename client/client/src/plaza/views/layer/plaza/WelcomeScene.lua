...
function WelcomeScene:onCreate()
    ...
    --获取本地保存config
--    local local_config = cc.UserDefault:getInstance():getStringForKey(LOCAL_CONFIG)
--    if local_config ~= "" then
--        appdf.Config_Infor = cjson.decode(local_config)
--    end

    --获取服务端config
--    if appdf.table_is_empty(appdf.Config_Infor) then
--        local configcallback = function (datatable,response)
--            appdf.Config_Infor = datatable
--            self:parseLogonUrl()
--        end
--        appdf.getNewConfigInfor(configcallback)
--    end
...
function WelcomeScene:parseLogonUrl()
    --解析登录地址
    local getLogonUrl = function (ret, url)
       if ret then
        appdf.Hall_Address = appdf.split(url, "|")
       end
        self:EnterClient()
    end
    decodeUrl(appdf.Config_Infor.body.game_hosts, getLogonUrl)
end
