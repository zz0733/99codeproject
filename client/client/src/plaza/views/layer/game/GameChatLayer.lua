...
function GameChatLayer:ctor(netframe, tabconfig)
    ...
    csbfile = tabconfig.csbfile or "public/GameChatLayer.csb"
    -- 资源检查
    if not cc.FileUtils:getInstance():isFileExist(csbfile) then
        csbfile = "public/GameChatLayer.csb"
    end
	--目前所以子游戏 聊天界面 没有修改 所以 默认公共的 
    if 50 ~= GlobalUserItem.nCurGameKind then --牛牛走传递的CSB
        csbfile = "public/GameChatLayer.csb"
    end
    --加载csb资源
	...
	self.recorde_chat_bg:setVisible(true)
...
function GameChatLayer:showTextChat(bShow)
    ...
    if nil ~= self.m_textTableView then
		...
		self.m_chat_recorde_region:setVisible(not bShow)
...
function GameChatLayer:getDataItemAt(view, idx)
    if CBT_TEXT == self.m_nSelect  then
        local chatitem = ChatItem:create(false, self.m_tabConfig)
        chatitem:setName(CHAT_ITEM_NAME)
        return chatitem
    elseif CBT_RECORD == self.m_nSelect then
        local chatitem = ChatItem:create(true, self.m_tabConfig)
        chatitem:setName(CHAT_ITEM_NAME)
        return chatitem
    end
    return nil
...
function ChatItem:ctor( bRecord, tabconfig )
    if not bRecord then
        local framename = tabconfig.szItemFrameName or "chat_item_framebg.png"
